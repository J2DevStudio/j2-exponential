            ...
            <nav className="h-full">
              {navItems.map(({ name, href }) => (
                <a
                  href={href}
                  key={href}
                  className="h-full inline-flex items-center px-10 relative font-bold text-xs tracking-widest text-gray-400 uppercase before:content-[''] before:absolute before:bottom-0 before:h-3 before:left-0 before:w-px before:bg-gray-200/20 after:content-[''] after:absolute after:bottom-0 after:h-3 after:right-0 after:w-px after:bg-gray-200/20 after:hidden last:after:block"
                >
                  {name}
                </a>
              ))}
